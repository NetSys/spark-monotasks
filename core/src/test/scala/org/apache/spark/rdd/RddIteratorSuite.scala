...
class RddIteratorSuite extends FunSuite with BeforeAndAfter with LocalSparkContext {
  ...
  before {
    ...
    context = new TaskContext(sc.env, null, 0, null, 0)
  ...
  test("iterator: get uncached local rdd") {
    rddA.persist(StorageLevel.MEMORY_ONLY)
    context = new TaskContext(sc.env, null, 0, null, 0, runningLocally = true)
  ...
  test("iterator: verify task metrics updated correctly") {
