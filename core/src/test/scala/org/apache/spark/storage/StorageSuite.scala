...
class StorageSuite extends FunSuite {
  ...
  private def storageStatus1: StorageStatus = {
    ...
    status.addBlock(TestBlockId("foo"), BlockStatus(memAndDisk, 10L, 20L, 1L, None))
    status.addBlock(TestBlockId("fee"), BlockStatus(memAndDisk, 10L, 20L, 1L, None))
    status.addBlock(TestBlockId("faa"), BlockStatus(memAndDisk, 10L, 20L, 1L, None))
    status
  }
  ...
  test("storage status update non-RDD blocks") {
    val status = storageStatus1
    status.updateBlock(TestBlockId("foo"), BlockStatus(memAndDisk, 50L, 100L, 1L, None))
    status.updateBlock(TestBlockId("fee"), BlockStatus(memAndDisk, 100L, 20L, 0L, None))
    assert(status.blocks.size === 3)
  ...
  private def storageStatus2: StorageStatus = {
    val status = new StorageStatus(BlockManagerId("big", "dog", 1), 1000L)
    assert(status.rddBlocks.isEmpty)
    status.addBlock(TestBlockId("dan"), BlockStatus(memAndDisk, 10L, 20L, 0L, None))
    status.addBlock(TestBlockId("man"), BlockStatus(memAndDisk, 10L, 20L, 0L, None))
    status.addBlock(RDDBlockId(0, 0), BlockStatus(memAndDisk, 10L, 20L, 1L, None))
    status.addBlock(RDDBlockId(1, 1), BlockStatus(memAndDisk, 100L, 200L, 1L, None))
    status.addBlock(RDDBlockId(2, 2), BlockStatus(memAndDisk, 10L, 20L, 1L, None))
    status.addBlock(RDDBlockId(2, 3), BlockStatus(memAndDisk, 10L, 20L, 0L, None))
    status.addBlock(RDDBlockId(2, 4), BlockStatus(memAndDisk, 10L, 40L, 0L, None))
    status
  }
  ...
  test("storage status update RDD blocks") {
    val status = storageStatus2
    status.updateBlock(TestBlockId("dan"), BlockStatus(memAndDisk, 5000L, 0L, 0L, None))
    status.updateBlock(RDDBlockId(0, 0), BlockStatus(memAndDisk, 0L, 0L, 0L, None))
    status.updateBlock(RDDBlockId(2, 2), BlockStatus(memAndDisk, 0L, 1000L, 0L, None))
    assert(status.blocks.size === 7)
    assert(status.rddBlocks.size === 5)
    assert(status.rddBlocksById(0).size === 1)
    assert(status.rddBlocksById(1).size === 1)
  ...
  test("storage status num[Rdd]Blocks") {
    ...
    status.addBlock(TestBlockId("Foo"), BlockStatus(memAndDisk, 0L, 0L, 100L, None))
    status.addBlock(RDDBlockId(4, 4), BlockStatus(memAndDisk, 0L, 0L, 100L, None))
    status.addBlock(RDDBlockId(4, 8), BlockStatus(memAndDisk, 0L, 0L, 100L, None))
    assert(status.blocks.size === status.numBlocks)
    assert(status.rddBlocks.size === status.numRddBlocks)
    ...
    status.updateBlock(TestBlockId("Foo"), BlockStatus(memAndDisk, 0L, 10L, 400L, None))
    status.updateBlock(RDDBlockId(4, 0), BlockStatus(memAndDisk, 0L, 0L, 100L, None))
    status.updateBlock(RDDBlockId(4, 8), BlockStatus(memAndDisk, 0L, 0L, 100L, None))
    status.updateBlock(RDDBlockId(10, 10), BlockStatus(memAndDisk, 0L, 0L, 100L, None))
    assert(status.blocks.size === status.numBlocks)
    assert(status.rddBlocks.size === status.numRddBlocks)
    assert(status.rddBlocksById(4).size === status.numRddBlocksById(4))
    assert(status.rddBlocksById(10).size === status.numRddBlocksById(10))
  ...
  test("storage status memUsed, diskUsed, tachyonUsed") {
    ...
    status.addBlock(TestBlockId("fire"), BlockStatus(memAndDisk, 4000L, 5000L, 6000L, None))
    status.addBlock(TestBlockId("wire"), BlockStatus(memAndDisk, 400L, 500L, 600L, None))
    status.addBlock(RDDBlockId(25, 25), BlockStatus(memAndDisk, 40L, 50L, 60L, None))
    assert(status.memUsed === actualMemUsed)
    assert(status.diskUsed === actualDiskUsed)
    assert(status.offHeapUsed === actualOffHeapUsed)
    status.updateBlock(TestBlockId("dan"), BlockStatus(memAndDisk, 4L, 5L, 6L, None))
    status.updateBlock(RDDBlockId(0, 0), BlockStatus(memAndDisk, 4L, 5L, 6L, None))
    status.updateBlock(RDDBlockId(1, 1), BlockStatus(memAndDisk, 4L, 5L, 6L, None))
    assert(status.memUsed === actualMemUsed)
    assert(status.diskUsed === actualDiskUsed)
    assert(status.offHeapUsed === actualOffHeapUsed)
  ...
  private def stockStorageStatuses: Seq[StorageStatus] = {
    ...
    status1.addBlock(RDDBlockId(0, 0), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    status1.addBlock(RDDBlockId(0, 1), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    status2.addBlock(RDDBlockId(0, 2), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    status2.addBlock(RDDBlockId(0, 3), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    status2.addBlock(RDDBlockId(1, 0), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    status2.addBlock(RDDBlockId(1, 1), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    status3.addBlock(RDDBlockId(0, 4), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    status3.addBlock(RDDBlockId(1, 2), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
  ...
  test("StorageUtils.getRddBlockLocations with multiple locations") {
    val storageStatuses = stockStorageStatuses
    storageStatuses(0).addBlock(RDDBlockId(1, 0), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    storageStatuses(0).addBlock(RDDBlockId(0, 4), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    storageStatuses(2).addBlock(RDDBlockId(0, 0), BlockStatus(memAndDisk, 1L, 2L, 0L, None))
    val blockLocations0 = StorageUtils.getRddBlockLocations(0, storageStatuses)
    val blockLocations1 = StorageUtils.getRddBlockLocations(1, storageStatuses)
    assert(blockLocations0.size === 5)
    assert(blockLocations1.size === 3)
